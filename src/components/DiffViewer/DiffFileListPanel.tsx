...
export function DiffFileListPanel() {
  ...
  const title = isBranchMode ? 'Branch Changes' : 'Changes';

  return (
    <div className="h-full bg-surface-2 flex flex-col">
      <PanelHeader title={title} badge={changedFilesCount} onRefresh={handleRefresh} />
      <div className="flex-1 overflow-y-auto">
        <DiffFileList
          status={status}
          onScrollToFile={handleScrollToFile}
          isLoading={isBranchMode ? branchIsLoading : statusLoading}
